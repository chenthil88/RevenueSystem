...
public class RevRecStageService {
    ...
    public int applyContractGrouping(long batchId, String groupingRuleSql) {
        return jdbc.update(buildApplyContractGroupingSql(groupingRuleSql),
    ...
    public Stream<RevRecStageRecord> streamUnassignedByGroupValue(long batchId) {
        return jdbc.queryForStream(STREAM_UNASSIGNED_BY_BATCH,
                new MapSqlParameterSource().addValue("batchId", batchId), rowMapper);
    }
    ...
    private static String buildApplyContractGroupingSql(String groupingRuleSql) {
        return """
                ...
                WHERE stg.`BatchId` = :batchId
                  AND IFNULL(stg.`processsedFlag`, 'N') IN ('N', 'E')
                """
                .formatted(
