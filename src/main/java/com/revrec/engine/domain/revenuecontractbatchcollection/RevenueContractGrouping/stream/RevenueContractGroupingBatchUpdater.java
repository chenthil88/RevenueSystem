...
public class RevenueContractGroupingBatchUpdater {
    ...
    public int bulkUpdateGroupingData(List<RevRecStageGroupingRecord> records) {
        ...
        for (int i = 0; i < records.size(); i += batchSize) {
            ...
            int[] updateCounts = jdbc.batchUpdate(
                    sql, batch.stream().map(this::buildParameterMap).toArray(SqlParameterSource[]::new));
    ...
    private SqlParameterSource buildParameterMap(RevRecStageGroupingRecord record) {
        return new MapSqlParameterSource()
                .addValue("id", record.getId())
                .addValue("tenantId", record.getTenantId())
                .addValue("revenueContractGroupValue", record.getRevenueContractGroupValue())
